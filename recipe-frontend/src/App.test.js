...
import { MemoryRouter } from 'react-router-dom'; // 使用 MemoryRouter 替换 BrowserRouter
...
describe('App Component', () => {
  ...
  it('renders RecipeList component for the root route', () => {
    render(
        <App />
    );

    const recipeListTitle = screen.getByText('My Recipe Book');
    expect(recipeListTitle).toBeInTheDocument();
  });

  it('renders RecipeDetail component for the /recipes/:id route', () => {
    render(
      <App location={{ pathname: '/recipes/1' }} />
    );

    const recipeDetailTitle = screen.getByText('Spaghetti Carbonara');
    expect(recipeDetailTitle).toBeInTheDocument();
  });
}
);
